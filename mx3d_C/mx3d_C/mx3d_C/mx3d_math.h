...
// How this works
// the mxHwnd object or the Master causes all change within the enviorment
// all the other objects simply keep a reference to this object and cause change
// This allows for complete flexibility and portability
// since all you gotta do is reimplement the mxHwnd class for the target platform
// and than everything should compile out fine.
// all the classes in this header file are independent of the OS except for the D3DCOLOR
// which can easily be typedef'd to unsigned long
...
namespace mx3d {
	...
	public:
		...
		std::string Print(std::string v = "v") {
			std::string p;
			char dat[256];
	...
	public:
		...
		virtual std::string Print(std::string v = "v") {
			std::string p;
			char dat[256];
	...
	public:
		...
		virtual std::string Print(std::string m = "m") {
			char dat[256];
			std::string p;
	...
	public:	
		...
		std::string Print(std::string m = "m") {
			std::string p;
			char dat[256];
			sprintf(dat,"%s = {\n", m.c_str());
	...
	enum { MX_ACTIVE = 0x1, MX_VISIBLE = 0x2, MX_BACKFACE = 0x4 , MX_CULLED = 0x5};
	...
	public:
		...
		std::vector<vec4D> local, trans;
		std::vector<Triangle> vlist;
		std::string object_name;

		// load my format
		bool LoadMX(std::string source, vec4D &scale, vec4D &obj_pos, vec4D &rotation) {
			std::fstream file;
		...
		// load andre's format
		bool LoadPLG(std::string plg,vec4D &scale, vec4D &obj_pos, vec4D &rotation) {
			std::fstream file;
			...
			while(!file.eof()) {
				...
				while(counter < num_p) {
					...
					Triangle tri;
					int num = 0;
					tri.color = D3DCOLOR_XRGB(rand()%255,rand()%255,rand()%255);// random color since it dosent specifiy
		...
		void TransformObject(Mat4D &mrot, int type=0) {
			switch(type) {
				case 0: // Translate Local Only
					...
					break;
				case 1: // Translate Trans Only
					{
						for(unsigned int i = 0; i < trans.size(); i++) {
							trans[i] = mrot.MulVec(trans[i]);
						}
					}
					break;
				case 2: // Local to Trans
					{
						for(unsigned int i = 0; i < local.size(); i++)
		...
		// the 4D vector pos, is the position of the camera preform the removeal of the faces
		void RemoveFaces(vec4D &pos) {
			for(unsigned int i = 0; i < this->vlist.size(); i++) {
				if(vlist[i].state == 0 || vlist[i].state & MX_VISIBLE || vlist[i].state & MX_CULLED ) // not active
					continue;
	...
	public:
		...
		void Begin(mxHwnd &mx) {
			this->mx = &mx;
			clip_min_x = clip_min_y  = min_clip_x = min_clip_y = 0;
			clip_max_x = this->mx->w - 1;
			clip_max_y = this->mx->h - 1;
			max_clip_x = this->mx->w - 1;
			max_clip_y = this->mx->h - 1;
			this->mx->paint.Lock();
		}
		...
		void DrawSolidPolys(RenderList &l) {
			...
			for(unsigned int i = 0; i < l.polys.size(); i++) {
				if(l.polys[i].state == 0 || l.polys[i].state == MX_BACKFACE )
					continue;
					this->mx->paint.drawtri(int(l.polys[i].tlist[0].x), int(l.polys[i].tlist[0].y), int(l.polys[i].tlist[1].x), int(l.polys[i].tlist[1].y), int(l.polys[i].tlist[2].x), int(l.polys[i].tlist[2].y),D3DCOLOR_XRGB(255,255,255));
			}
		}

		void DrawObject(mxObject &o) {

			if(o.state == MX_CULLED) // if its not visible on the screen than dont draw it!
				return;
		...
		virtual void DrawClipedLine(int x1, int y1, int x2, int y2, D3DCOLOR color) {
			if(ClipLine(x1,y1,x2,y2))
			{
				mx->paint.drawline(x1,y1,x2,y2,color);
			}	
		}
