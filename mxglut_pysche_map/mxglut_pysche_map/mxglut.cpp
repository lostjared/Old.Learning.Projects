...
namespace mx {
	...
	bool mxGlut::UnBindTextures(mx::mxTexture &tex, GLuint *tex_array, GLuint index) {
		glDeleteTextures(index, tex_array);
		return true;
	}
	...
	void mxTexture::ScrambleData() {
		GLubyte *raw_bits = this->GetRawData();
		for(int loop = 0; loop < ((this->width * this->height)-8); loop++) {
			// set the bits
			*raw_bits = *(raw_bits+8);
			raw_bits++;
			*raw_bits = *(raw_bits+4);
