...
public:
	...
	void PrintTexturedText(mxText *tex, int x, int y, int w, int h,const char *str, ...) {
		int prev_x = x;
		int offset_x = prev_x, offset_y = y;
		int width = 0, height = 0;
		char text[5000];// just enough
		...
		for(unsigned int p = 0; p < strlen(text); p++) {
			...
			for(int i = 0; i < h; i++) {
				for(int z = 0; z < w; z++) {
					if(tex->GetFont()->letters[text[p]].fnt_ptr[GetFX(tex->GetFont(),i,w)][GetFZ(tex->GetFont(),z,h)] != tex->GetFont()->tcolor) {
						SetPixel(&front, offset_x+i, offset_y+z, this->GetPixel(tex->surf, GetFX(tex->GetFont(),i, w), GetFZ(tex->GetFont(),z, h)));
						width=i;
						if(height < z)
							height=z;
					}
