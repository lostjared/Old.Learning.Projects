...
int SDL_main(int argc, char *argv[])
{
    ...
    srand(time(0));
	if(mxhwnd.init("HPPD in SDL w/ Joystick Support",480,272,0,false) == 0)
	{     
...
void update_screen(int scr)
{
    ...
    if(init_a == false)
    {
        for(int i = 0; i < 480*272; i++)
        {
            ...
            array_of_pixels[i].setDirection(rand()%480,rand()%272,rand()%4);                
        }
	...
	unlock(mxhwnd.pscr);       
       ...
       update_cords();
        ...
        SDL_UpdateRect(mxhwnd.pscr,0,0,480,272);
...
void shift_up()
{
 for(int i = 0; i < 480*272; i++)
 {
     if(array_of_pixels[i].x_pos >= 0 && array_of_pixels[i].x_pos <= 480 && array_of_pixels[i].y_pos >= 440 && array_of_pixels[i].y_pos <= 272)
     {
         array_of_pixels[i].setDirection(array_of_pixels[i].x_pos,array_of_pixels[i].y_pos,rand()%6);           
}     
}  
...
void shift_down()
{
    ...
    for(int i = 0; i < 480*272; i++)
 {
     if(array_of_pixels[i].x_pos >= 0 && array_of_pixels[i].x_pos <= 480 && array_of_pixels[i].y_pos >= 0 && array_of_pixels[i].y_pos <= 40)
     {
         array_of_pixels[i].setDirection(array_of_pixels[i].x_pos,array_of_pixels[i].y_pos,rand()%6);           
}     
}   
