...
void FAST MasterText::printtext(char* strtext, int x, int y)
{
	...
	TextOut(dc,x,y,strtext,static_cast<int>(strlen(strtext)));
...
void FAST MasterText::printtextrect(char* strtext, int y, int x, int w, int h)
{
	...
	DrawText(dc,strtext,static_cast<int>(strlen(strtext)),&rt,DT_EDITCONTROL);
...
// print formated text rectangle 
void FAST MasterText::printtextrectf(int x, int y, int w, int h, char * str, ...)
{
	va_list list; 
	char string[5000];
	va_start(list, str); 
