...
long XSTD event(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	case MASTER_OUT:
		{
			int len = flen("c:\\stream.o");
			if(len == -1) { break; }
			char* buf = new char [ len + 1 ];
			readfile( "c:\\stream.o",buf );
			console << buf;
			delete [] buf;
			buf = NULL;
	...
	case MASTER_IN:
		{
			console.console_input = true; 
			console.input();
		}
		break;
		// the program is done executing, return control back to the troll under the bridge
	case MASTER_DONE:
		{
			console.nextcmd();
		}
	...
	case MASTER_PAUSE:
		...
		// initlize the program, get the handle of the window
		// to send messages to
	case MASTER_INIT:
		{
			console.ioff();
