...
extern mxJoystick stick;
extern LRESULT joy_enabled;
...
private:
	...
	inline void logic()
	{
		...
		if(mxhwnd.KeyCheck(DIK_LEFT) || joy_enabled && stick.joy_st.lX < -700)
		{
			heroleft();
		}
		
		if(mxhwnd.KeyCheck(DIK_RIGHT) || joy_enabled && stick.joy_st.lX > 700)
		{
			heroright();
			
		}
		
		if(mxhwnd.KeyCheck(DIK_A) || joy_enabled && stick.joy_st.rgbButtons[1] & 0x80)
		{
			if(air == false)
			{
		...
		if(mxhwnd.KeyCheck(DIK_S) || joy_enabled && stick.joy_st.rgbButtons[2] & 0x80)
		{
